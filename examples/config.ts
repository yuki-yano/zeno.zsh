...
export default defineConfig(({ projectRoot, currentDirectory, env, shell }) => {
    ...
    completions.push(
      {
        ...
        patterns: ["^npm run\\s"],
        sourceCommand:
          "cat package.json 2>/dev/null | jq -r '.scripts | keys[]' 2>/dev/null",
        options: {
          "--prompt": "'NPM Scripts> '",
          "--preview": "'cat package.json | jq -r \".scripts.{}\" 2>/dev/null'",
        },
        callback: "awk '{print $1}'",
      },
      {
  ...
  if (isKubernetesProject) {
    completions.push(
      {
        name: "kubectl pods",
        patterns: ["^kubectl (logs|exec|describe|delete pod|port-forward)\\s"],
        sourceCommand:
